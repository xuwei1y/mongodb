...
%% @doc  Fresh object id add one extra data
-spec object_id ( ExtraData::binary()) -> bson:objectid(). % IO
object_id(ExtraData) when is_binary(ExtraData) ->
	Now = bson:unixtime_to_secs (bson:timenow()),
	MPid = ets:lookup_element (?MODULE, oid_machineprocid, 2),
	N = ets:update_counter (?MODULE, oid_counter, 1),
	bson:objectid(Now, MPid, N , ExtraData ).

...
-spec set_connection(pid() , pid()) -> ok |error .
set_connection(Cursor , Worker ) when is_pid(Worker ) ->
  try gen_server:cast(Cursor, { set_connection , Worker })
  catch
    exit:{noproc, _} -> error
  end.
...
%% @hidden
handle_cast({ set_connection , Worker } , State) ->
  {noreply, State#state{ connection = Worker }} ;
